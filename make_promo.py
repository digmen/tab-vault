...
img.save('store/promo_small.png')
print('promo_small.png saved')
...
for gx in range(0, W, 60):
    for gy in range(0, H, 60):
        d.ellipse([gx, gy, gx + 1, gy + 1], fill=(255, 255, 255, 7))
...
# lock icon (small rectangle + arch)
lx2, ly2 = LX + 12, fy + 16
...
for sy in range(44, H - 44):
    ...
    a  = int(60 * t)
    if a > 0:
        d.point((SEP, sy), fill=(ACC[0], ACC[1], ACC[2]))

# ── RIGHT PANEL: popup mockup ─────────────────────────────────
RX = SEP + 52
RY = 24
PW = 398
PH = H - 48
...
d.line([RX, RY + HDR_H, RX + PW, RY + HDR_H], fill=(255, 255, 255, 14), width=1)
...
for idx, (state, sc, row_bg, group_color, url, title) in enumerate(rows):
    ...
    if group_color:
        d.rounded_rectangle([RX + 10, ty, RX + 14, ty + ROW_H],
                            radius=2, fill=group_color)

    d.rounded_rectangle([RX + 14, ty, RX + PW - 12, ty + ROW_H],
                        radius=9, fill=rb, outline=bdc, width=1)
    ...
    if is_hover:
        for bi, (btxt, bcol) in enumerate([('↗', ACCL), ('×', RED)]):
            ...
            d.rounded_rectangle([bx2, ty + 12, bx2 + 28, ty + 40],
                                radius=7, fill=(30, 26, 56), outline=bdc, width=1)
...
# footer
FY = RY + PH - 52
d.line([RX, FY, RX + PW, FY], fill=(255, 255, 255, 12), width=1)

pb_w = (PW - 38) // 2
# save
d.rounded_rectangle([RX + 12, FY + 9, RX + 12 + pb_w, FY + 42], radius=8, fill=(8, 32, 16))
label = 'Save current tab'
d.text((RX + 12 + (pb_w - txt_w(d, label, f_btn)) // 2, FY + 18),
       label, font=f_btn, fill=GRN)
# restore
rx3 = RX + 26 + pb_w
d.rounded_rectangle([rx3, FY + 9, rx3 + pb_w, FY + 42], radius=8, fill=(22, 18, 54))
label2 = 'Restore all'
d.text((rx3 + (pb_w - txt_w(d, label2, f_btn)) // 2, FY + 18),
       label2, font=f_btn, fill=ACCL)

# accent bar
d.rectangle([0, H - 5, W, H], fill=ACC)

img.save('store/promo_large.png')
print('promo_large.png saved')

for fn in ['store/promo_small.png', 'store/promo_large.png']:
    im = Image.open(fn)
